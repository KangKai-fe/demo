...
var H5ComponentBase =function ( name, cfg ) {
    ...
    component.on('onLoad',function(){
        ...
        if (cfg.relativeTo) {
            var $target = $('.h5_component_name_' + cfg.relativeTo);

            if (!cfg.center) {
                component.css('transform', 'translate(' + $target.offset().left + 'px, ' + $target.offset().top + 'px)');
            } else {
                component.css('transform', 'translateY(' + $target.offset().top + 'px)');
            }
        }
        setTimeout(function(){
